...
public static void main(String[] args) throws java.lang.Exception{
		...
		for(int t=1;t<=m;t++){
			int a = i(); int b = i();
			adj[a].add(b); //only for directional graph
			
		}
		boolean ans = false;
		for(int i=1;i<=n;i++){
		    if(!visited[i])ans = ans | dfs(adj,i,visited,n); //or   ... the Graph is forest contains disconnected components 
		}
		if(ans)out.write("yes");else out.write("no cycle");
		out.flush();
...
public static boolean dfs(LinkedList<Integer> adj[] ,int root, boolean [] visited ,int n)throws Exception{
