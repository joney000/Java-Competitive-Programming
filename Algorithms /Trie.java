 ...
 *  @TimeStamp : Sunday, August 21 2016 : 10:49 AM
 *  @EventLog  : Mid Day Coding
 */
 ...
 //	public boolean isLeaf = false;
 	...
 	public HashMap <Character,Node> children = new HashMap<Character,Node>(2);
 	@Override
 	public String toString(){
 		return ""+this.prefix_count + " " + exact_count + " children: "+ children;
 	}
 }
 class A {
	...
	*/
	...
	//   for(int t = 1 ; t <= tests ; t++){
		...
		add(root, String.format("%30s", Integer.toBinaryString(0)).replace(' ', '0'), 0);
		
		for(int i = 1 ; i <= n ; i++){
			String []word = s().split(" ");
			String type = word[0];
			int x = Integer.parseInt(word[1]);
			String s = String.format("%30s", Integer.toBinaryString(x)).replace(' ', '0');
					
			// Node : curr, word : String , idx : charAt
			if(type.equals("+")){
				add(root, s, 0);
			}else if(type.equals("-")){
				remove(root,s,0);
				...
				ans = calculate(root,s,0,ans);
				out.write(""+Integer.parseInt(ans.toString(),2)+"\n");
			//	out.flush();
			}
	...
	void add(Node curr, String word, int pos){
		if(pos >= word.length())return; 	// Invalid
		char ch = word.charAt(pos);
		Node child = curr.children.get(ch);
		if(child == null){
			child  = new Node();
	//		child.isLeaf = true;
		}
		child.prefix_count += 1;
	//	if(pos== word.length()-1)child.exact_count += 1;
		curr.children.put(ch,child);
	//	
	//	out.write("for ch="+ch+" pos="+pos+"\n");
	//	out.flush();
		add(child, word, pos+1);
	}
	
	void remove(Node curr, String word, int pos){
		if(pos >= word.length())return ;	// Invalid
		char ch = word.charAt(pos);
		Node child = curr.children.get(ch);
		child.prefix_count -= 1;
	//	if(pos== word.length()-1)child.exact_count -= 1;
		curr.children.put(ch,child);
		//out.write("for ch="+ch+" and child : "+child+"\n");
	//	out.flush();
		remove(child, word, pos+1);
	}
	
	StringBuilder calculate(Node curr, String word, int pos , StringBuilder ans){				// This is the function logics as per problem 
		if(pos >=word.length())return ans; 	// Invalid
		char ch = word.charAt(pos);
		int chh = ch - '0';
		chh ^= 1;
		char ich = (char)('0' + chh);
		Node child = curr.children.get(ich);
		if(child != null && child.prefix_count > 0)ans.append('1');
		else {
			child = curr.children.get(ch);
			ans.append('0');
			}
		//out.write("here :"+ans+" ch="+ch+"\n");
		//out.flush();
		return calculate(child,word,pos+1,ans);
	}
