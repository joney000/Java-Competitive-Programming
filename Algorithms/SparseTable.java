...
class E
{
  ...
  public E(int[] a ,int n) {
	...
	rmq = new int[SparseTable[n]+1][n];
    // Initializing table O(<nlogn>,<1>)
	for(int i = 0;i<n; i++){
		rmq[0][i] = i;
	}
    for (int k = 1; (1<<k)< n ; k++) {
  ...
  public int min(int i, int j) {
	...
	*/
	...
	public static void main(String[] args) throws java.lang.Exception{
		/*
		 * Spacifications : 0 based indexing.
		 *
		 */		 
			
		int n = i();int q =i();
	
		for(int x=0;x<=n-1;x++)arr[x] = i();
		E st = new E(arr,n);
		for(int i =1 ;i<=q ;i++){
			int l = i(); int r = i();
			out.write(" Li="+l+" R[i]="+r+" "+((st.min(l-1,r-1) ))+"\n");
			
		}
		for(int i = 0; (1<<i)< n ; i++){
			for(int j = 0 ; j < n ; j++)out.write(""+st.rmq[i][j]+" ");
			out.write("\n");
		}
