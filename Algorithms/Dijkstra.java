...
import java.math.*;
/*
 * Author    : joney_000[let_me_start][jaswantsinghyadav007@gmail.com]-Jaswant Singh
 * Algorithm : Dijkstra
 * Platform  : Codeforces
 * Lang      : Java8
 * Date      : 3/march/2015
...
class Edge implements Comparable<Edge> {
     ...
     public long weight;
     public long diff; 
     //public provide flexibility to access from outside the the class/package
...
class A{
	...
	*/
    ...
    private int source, n , k , x , m , s;// N vertices and M edeges
    private long d[] = new long[100005]; 
  	...
  	void run()throws Exception{
		...
		for(int t  = 1 ; t<= tests ; t++){
            ...
            for(int i = 1; i <= m; i++){
                int u  = i(); int v  = i(); long w = i();
                adj[u].add(new Edge(v,w));              
                adj[v].add(new Edge(u,w));//if the graph is undirected
            }
            djkstra(adj,s,n);
            for(int i = 1; i <= n; i++)out.write(""+d[i]+" ");
            out.write("\n");
		}//end tests
	}//end run
	...
	void clear(){
       isCliqueVis = false;
       Arrays.fill(d, 1, 100001, INF_L);    // fill(arr[], int st, int end, long val) range = [1,n)
	}

    void djkstra(ArrayList<Edge> []adj,int source,int n)throws Exception{
    ...
    void fill_clique(int entry_point, PriorityQueue<Edge> pq){
        for(int v = 1; v <= k; v++){
            if(v == entry_point)continue;
            if(d[entry_point] + x < d[v]){
                d[v] = d[entry_point] + x;
                pq.add(new Edge(v,d[v]));
            }
        }
    ...
    void printDistances(int source,long []d,int n)throws Exception{
